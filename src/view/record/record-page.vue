<template>
  <div>
    <div class="search-con search-con-top">
      <label>&nbsp;&nbsp;用户ID:&nbsp;&nbsp;</label>
      <Input clearable
             placeholder="用户ID"
             class="search-input"
             v-model="search.userId" />
      <label>&nbsp;&nbsp;用户类型:&nbsp;&nbsp;</label>
      <Select v-model="search.userType"
              placeholder="全部"
              class="search-input">
        <Option :value="''">全部</Option>
        <Option :value="'user'">后台管理员</Option>
        <Option :value="'app'">微服务</Option>
      </Select>

      <label>&nbsp;&nbsp;操作类型:&nbsp;&nbsp;</label>
      <Select v-model="opAction"
              placeholder="全部"
              class="search-input">

        <Option :value="''">全部</Option>
        <Option :value="key"
                :key="key+'-'+desc"
                v-for="(desc,key) in opTypeMap">{{desc}}</Option>
      </Select>

      <label>&nbsp;&nbsp;操作时间:&nbsp;&nbsp;</label>

      <DatePicker type="datetime"
                  placeholder="操作起始时间"
                  :value="search.startTime"
                  @on-change="search.startTime=$event"
                  style="width: 180px"></DatePicker>
      ~
      <DatePicker type="datetime"
                  placeholder="操作结束时间"
                  :value="search.endTime"
                  @on-change="search.endTime=$event"
                  style="width: 180px"></DatePicker>
      <Button @click="handleSearch"
              class="search-btn"
              type="primary">
        <Icon type="search" />&nbsp;&nbsp;搜索</Button>

    </div>

    <Table :data="tableData"
           height="550"
           :loading="loading"
           :columns="tableColumns"
           stripe
           border>
      <template slot-scope="{ row }"
                slot="opTime">
        {{new Date(row.opTime).toLocaleString()}}

      </template>
      <template slot-scope="{ row }"
                slot="opUser">
        {{userTypeDesc[row.userType]+':'+row.userId }}

      </template>
      <template slot-scope="{ row }"
                slot="opType">
        {{opTypeDesc[row.opType]+" - "+tableDesc[row.table]}}

      </template>
      <template slot-scope="{ row }"
                slot="opDesc">
        {{getRecordDesc(row)}}
      </template>
    </Table>
    <div style="margin: 10px;overflow: hidden">
      <div style="float: right;">
        <Page :total="total"
              :current="search.curPageNum"
              :page-size="search.pageSize"
              show-total
              show-elevator
              show-sizer
              transfer
              @on-page-size-change="changePageSize"
              @on-change="changePageIndex"></Page>
      </div>
    </div>

  </div>
</template>
<script>
import { getRecordList, getRecordCount } from '@/api/record'
import expandRow from './table-expand.vue'

export default {
  components: { expandRow },

  data () {
    return {
      loading: true,
      tableData: [],
      opAction: '',
      tableColumns: [
        {
          type: 'expand',
          width: 60,
          title: '详情',
          align: 'center',
          render: (h, params) => {
            return h(expandRow, {
              props: {
                record: params.row
              }
            })
          }
        },
        {
          title: '操作时间',
          align: 'center',
          slot: 'opTime',
          width: 170
        },
        {
          title: '操作人',
          align: 'center',
          slot: 'opUser',
          width: 300
        },
        {
          title: '操作类别',
          align: 'center',
          slot: 'opType',
          width: 150
        },
        {

          title: '操作内容',
          // align: 'center',
          slot: 'opDesc'
        }
      ],
      total: 0,

      search: {
        pageSize: 10,
        curPageNum: 1,
        userId: '',
        userType: '',
        table: '',
        opType: '',
        startTime: '',
        endTime: ''
      },
      opTypeDesc: {
        'delete': '删除',
        'add': '添加',
        'update': '修改'
      },
      userTypeDesc: {
        'user': '管理员',
        'app': '微服务'
      },
      tableDesc: {
        'power': '权限配置',
        'role': '角色配置',
        'user': '用户　　',
        'appInfo': '微服务　',
        'platform': '项目信息',
        'roleBind': '角色绑定',
        'project': '平台'
      }
    }
  },
  methods: {

    getRecordDesc (record) {
      let desc = ``

      let id
      let arrName = 'id'
      if (record.table === 'user') {
        arrName = 'userId'
      } else if (record.table === 'appInfo') {
        arrName = 'key'
      } else if (record.table === 'platform') {
        arrName = 'platformId'
      } else if (record.table === 'project') {
        arrName = 'name'
      }

      switch (record.opType) {
        case 'add':
          id = record.content[arrName]
          break
        case 'delete':
          id = (record.content[arrName] && record.content[arrName].$in && record.content[arrName].$in[0]) || record.content[arrName]
          break
        case 'update':
          id = record.content.query[arrName]
      }
      desc += ` id:${id}`
      return desc
    },

    getSearch () {
      let search = this.search
      search.table = ''
      search.opType = ''
      if (this.opAction !== '') {
        let split = this.opAction.split('-')
        search.opType = split[0]
        search.table = split[1]
      }
      return search
    },

    getRecordList () {
      this.loading = true

      return getRecordList(this.getSearch()).then(res => {
        let data = res.data
        if (data.errcode === '0') {
          this.loading = false
          data = data.object
          this.tableData = data
        } else {
          this.$Message.error(data.errmsg)
        }
      }).catch(e => {
        this.$Message.error(e.toString())
      })
    },
    getRecordCount () {
      return getRecordCount(this.getSearch()).then(res => {
        let data = res.data
        if (data.errcode === '0') {
          data = data.object
          this.total = data.count
        } else {
          this.$Message.error(data.errmsg)
        }
      }).catch(e => {
        this.$Message.error(e.toString())
      })
    },

    changePageSize (pageSize) {
      this.search.pageSize = pageSize
      this.getRecordList()
    },

    changePageIndex (curPageNum) {
      this.search.curPageNum = curPageNum
      this.getRecordList()
    },

    fetchData () {
      return Promise.all([this.getRecordCount(), this.getRecordList()]).then(() => {

      })
    },
    handleClear () {

    },

    handleSearch () {
      this.fetchData()
    }
  },
  computed: {
    opTypeMap () {
      let m = {}
      for (let opTypeKey in this.opTypeDesc) {
        for (let tableKey in this.tableDesc) {
          m[opTypeKey + '-' + tableKey] = this.opTypeDesc[opTypeKey] + '-' + this.tableDesc[tableKey]
        }
      }

      return m
    }
  },
  mounted () {
    return this.fetchData()
  }
}
</script>
<style  lang="less">
.search-con {
  padding: 10px 0;
  .search {
    &-col {
      display: inline-block;
      width: 100px;
    }

    &-status {
      display: inline-block;
      width: 100px;
      margin-left: 2px;
    }
    &-input {
      display: inline-block;
      width: 130px;
      margin-left: 2px;
    }
    &-btn {
      margin-left: 5px;
    }
  }
  .new-btn {
    margin-left: 5px;
  }
}
</style>
